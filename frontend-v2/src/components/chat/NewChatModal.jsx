    ...
    return (
        ...
        >
            <div className="flex flex-col flex-1 min-h-0 space-y-4 py-2">
                <div>
                    <label className="text-[10px] uppercase font-medium tracking-wider text-gray-500 mb-1.5 block px-1">
                        Search Users
                    </label>
                    <Input
                        id="new-chat-search"
                        name="userSearch"
                        placeholder="Type an email or contact…"
                        value={searchQuery}
                        onChange={(e) => handleSearch(e.target.value)}
                        className="h-10 bg-white/3 border-white/5 focus:border-blue-500/40"
                        autoFocus
                        leftIcon={
                ...
                <div className="flex-1 min-h-50 overflow-y-auto custom-scrollbar space-y-1.5">
                    {isSearching ? (
                        <div className="flex flex-col items-center justify-center py-12">
                            <div className="w-6 h-6 border-2 border-white/10 border-t-white/60 rounded-full animate-spin mb-2" />
                            <span className="text-[10px] font-medium text-gray-500">Searching…</span>
                        </div>
                        ...
                        searchResults.map((user) => (
                            <button
                                ...
                                className="flex items-center gap-3 p-3 rounded-xl bg-white/2 border border-white/5 hover:border-blue-500/20 hover:bg-white/4 transition-all w-full text-left group"
                            >
                                ...
                                <svg width="12" height="12" viewBox="0 0 15 15" fill="none" className="text-gray-500 group-hover:text-blue-400 transition-colors">
